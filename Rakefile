#!/usr/bin/rake
## Based on the code from Sourcery https://github.com/krzysztofzablocki/Sourcery/blob/master/Rakefile
...
task :tests do
  ...
  xcrun %Q(xcodebuild -workspace IBAnalyzer.xcworkspace -scheme IBAnalyzer -sdk macosx test)
end
...
task :build do
  ...
  xcrun %Q(xcodebuild -workspace IBAnalyzer.xcworkspace -scheme IBAnalyzer -configuration Release -sdk macosx -derivedDataPath #{BUILD_DIR}/tmp/)
  sh %Q(rm -fr bin/IBAnalyzer.app)
  `mv #{BUILD_DIR}tmp/Build/Products/Release/IBAnalyzer.app bin/`
...
namespace :release do
  ...
  task :check_versions do
    ...
    # Extract version from IBAnalyzer.podspec
    version = podspec_version
    puts "#{'IBAnalyzer.podspec'.ljust(25)} \u{1F449}  #{version}"
  ...
  task :zip => [:clean] do
    ...
    `cd build; zip -r -X ibanalyzer-#{podspec_version}.zip .`
  end
  ...
  def post(url, content_type)
    ...
    req.basic_auth 'fastred', File.read('.apitoken').chomp
  ...
  task :github => :zip do
    ...
    json = post('https://api.github.com/repos/fastred/IBAnalyzer/releases', 'application/json') do |req|
      req.body = { :tag_name => v, :name => v, :body => changelog, :draft => false, :prerelease => false }.to_json
    end

    upload_url = json['upload_url'].gsub(/\{.*\}/,"?name=IBAnalyzer-#{v}.zip")
    zipfile = "build/IBAnalyzer-#{v}.zip"
  ...
  desc 'pod trunk push IBAnalyzer to CocoaPods'
  task :cocoapods do
    print_info "Pushing pod to CocoaPods Trunk"
    sh 'bundle exec pod trunk push IBAnalyzer.podspec --allow-warnings'
  end
end
